...
def main():
    ...
    len_int=[int(ele) for ele in numpy.linspace(1, 5, 5)]

    BER_coh = numpy.zeros((len(len_int), len(EbN0dB)))

    #Simu
    for l in range(0, len(len_int)):
        print('Phase integrator length: ' + str(len_int[l]))
        for i in range(0, len(EbN0dB)):
            print("Eb/N0=" + str(EbN0dB[i]))

            n_err_coh = 0
            for j in range(0, n_simus):
                tb = ber_vs_ebn0_awgn(M, n_syms, len_int[l], EbN0dB[i])
                tb.run()
            ...
            BER_coh[l][i] = float(n_err_coh)/(n_simus*n_syms*SF)

            print("Error count:" + str(n_err_coh))
            print("BER:" + str(BER_coh[l][i]))
            print("\n")

    #Plot results
    for l in range(0, len(BER_coh)):
        plt.semilogy(EbN0dB, BER_coh[l], '-+', label="len_int =" + str(len_int[l]))
