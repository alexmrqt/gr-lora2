...
def main():
    ...
    EbN0dB.tofile(fid)
    fid.close()
    fid = open("MFSK_BER.raw", "wb")
    BER_awgn['uncoh'].tofile(fid)
    fid = open("MFSK_BER_coh.raw", "wb")
    BER_awgn['coh'].tofile(fid)
    fid = open("MFSK_BER_f_chan.raw", "wb")
    BER_proakis_b['uncoh'].tofile(fid)
    fid = open("MFSK_BER_coh_f_chan.raw", "wb")
    BER_proakis_b['coh'].tofile(fid)
    fid = open("MFSK_BER_t_chan.raw", "wb")
    BER_basic_t_sel['uncoh'].tofile(fid)
    fid = open("MFSK_BER_coh_t_chan.raw", "wb")
